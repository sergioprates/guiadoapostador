...
namespace GuiaDoApostadorDominio.Controllers
{
    public class LotofacilController : ILotofacilController
    {
        ...
        {
            int sorteiosAnteriores = 3;
            int quantidadePalpite = 20;
            Dictionary<byte, byte> numerosLoteria = new Dictionary<byte, byte>(Lotofacil.TotalNumerosLoteria);
            Dictionary<byte, byte> ultimosNumeros = _repository.GeraPalpiteProxSorteio(sorteiosAnteriores);

            for (byte i = 1; i <= Lotofacil.TotalNumerosLoteria; i++)
            {
            ...
            while (palpite.Count != Lotofacil.QuantidadePadraoAposta)
            {
